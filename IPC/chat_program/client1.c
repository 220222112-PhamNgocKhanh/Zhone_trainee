...
/* Thread function to continuously receive messages from Client 2 */
...
int main(void)
{
    ...
    printf("[Client 1] Waiting for Client 2 to connect...\n");

    fd_write = open(FIFO_1_TO_2, O_WRONLY);
    if (fd_write < 0)
    {
    ...
    fd_read = open(FIFO_2_TO_1, O_RDONLY);
    if (fd_read < 0)
    {
